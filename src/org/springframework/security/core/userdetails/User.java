...
import com.baodian.service.util.impl.StaticDataManagerImpl;
...
public class User implements UserDetails, CredentialsContainer {
    ...
    public Collection<GrantedAuthority> getAuthorities() {
    	if(this.authorities == null) {//延迟读取用户角色等信息
			...
			com.baodian.model.user.User user = SecuManagerImpl.findUserR_ByU_a(username);
			int did = user.getDpm().getId();
			//用户id
			this.id = new int[] {user.getId(), did, StaticMethod.getDPidonDt(did)};
			//用户真实姓名、头像
	        ...
	        if(!rids.isEmpty())
	        	userMenu.addAll(SecuManagerImpl.findMenuByRIds(rids.substring(0, rids.length()-1)));
    ...
    public String getDepartment() {
    	return StaticDataManagerImpl.depts.get(id[1]).getName();
    }
